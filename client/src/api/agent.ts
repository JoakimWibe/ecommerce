...
axios.defaults.baseURL = "http://localhost:5284/api/";
...
axios.interceptors.response.use(async response => {
    ...
    const {data, status} = errror.response as AxiosResponse;
    switch (status) {
        case 400:
            if (data.errors) {
    ...
    return Promise.reject(errror.response);
})
...
const Catalog = {
    ...
    details: (id: null) => request.get(`products/${id}`)
}
